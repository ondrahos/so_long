...
static void	init_texture(t_struct *game)
{
	...
	game->collectible_img = mlx_texture_to_image
		(game->mlx_ptr, game->collectible);
...
static void	load_texture(t_struct *game, int y, int x)
{
	if (game->map[y][x] == '1')
		if (mlx_image_to_window(game->mlx_ptr, game->wall_img,
				x * 64, y * 64) < 0)
			exit_error(game, 4);
	if (game->map[y][x] == '2' || game->map[y][x] == 'c'
		|| game->map[y][x] == 'p'
		|| game->map[y][x] == 'e')
		if (mlx_image_to_window(game->mlx_ptr, game->floor_img,
				x * 64, y * 64) < 0)
			exit_error(game, 4);
	if (game->map[y][x] == 'c')
		if (mlx_image_to_window(game->mlx_ptr, game->collectible_img,
				x * 64, y * 64) < 0)
			exit_error(game, 4);
	if (game->map[y][x] == 'e')
		if (mlx_image_to_window(game->mlx_ptr, game->horse_img,
				x * 64, y * 64) < 0)
			exit_error(game, 4);
}
...
void	load_all_textures(t_struct *game)
{
	...
	while (game->map[y])
	{
		...
		while (game->map[y][x])
		{
			load_texture(game, y, x);
			x++;
		}
		y++;
	}
	game->character_img = mlx_texture_to_image(game->mlx_ptr, game->character);
	if (mlx_image_to_window(game->mlx_ptr, game->character_img,
			game->x * 64, game->y * 64) < 0)
		exit_error(game, 4);
	if (!game->floor_img || !game->wall_img || !game->collectible_img
		|| !game->character_img || !game->horse_img)
		exit_error(game, 4);
}
